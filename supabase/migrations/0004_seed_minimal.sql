...
BEGIN
  ...
  IF tbl IS NULL THEN
    RAISE NOTICE 'public.roles not found; skipping seed';
    RETURN;
  END IF;
  ...
  ELSIF has_name THEN
    idcol := 'name';
  ELSE
    RAISE NOTICE 'no suitable identifier column (slug/code/key/name); skipping seed';
    RETURN;
  END IF;

  FOR r IN SELECT unnest(ARRAY['guardian','doctor','dietitian','admin'])
  LOOP
    EXECUTE format('SELECT EXISTS(SELECT 1 FROM public.roles WHERE %I = $1)', idcol)
      INTO exists_row USING r;

    IF NOT exists_row THEN
      IF has_label THEN
        EXECUTE format('INSERT INTO public.roles(%I, label) VALUES ($1, initcap($1))', idcol)
          USING r;
      ELSIF has_name AND idcol <> 'name' THEN
        EXECUTE format('INSERT INTO public.roles(%I, name) VALUES ($1, initcap($1))', idcol)
          USING r;
      ELSE
        EXECUTE format('INSERT INTO public.roles(%I) VALUES ($1)', idcol)
          USING r;
      END IF;
    END IF;
  END LOOP;
